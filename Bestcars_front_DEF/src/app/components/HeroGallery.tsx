    ...
    processedImages.horizontal.forEach((image, index) => {
      slides.push(
        <div key={`horizontal-${index}`} className="single-image-slide">
          <img src={image} alt={`Vehículo de lujo Best Cars Ibérica — foto ${index + 1}`} loading="lazy" crossOrigin="anonymous" style={{ aspectRatio: '3/2' }} />
        </div>
      );
    });
          ...
          <div className="vertical-images-grid">
            {group.map((image, imageIndex) => (
              <div key={imageIndex} className="vertical-image-container">
                <img src={image} alt={`Vehículo de lujo Best Cars Ibérica — foto ${groupIndex * 3 + imageIndex + 1}`} loading="lazy" crossOrigin="anonymous" style={{ aspectRatio: '2/3' }} />
              </div>
            ))}
          </div>
        </div>
